...
async function searchResults(keyword) {
  try {
    ...
    // Define a desktop user agent to reduce the chance of being blocked by
    // Cloudflare or other protection layers.  Passing a referer header helps
    // ensure the server responds with full HTML.
    const headers = {
      'User-Agent':
        'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
      Referer: 'https://ogomovies.com.pk/'
    };
    ...
    const searchUrl =
      `https://ogomovies.com.pk/wp-json/wp/v2/search?search=${encoded}&subtype=movies&per_page=10`;
    ...
    for (const item of data) {
      ...
      if (href) {
        ...
        try {
          const pageRes = await fetchv2(href, headers);
...
async function extractDetails(url) {
  try {
    ...
    if (slug) {
      ...
      const res = await fetchv2(apiUrl, headers);
...
async function extractStreamUrl(url) {
  try {
    ...
    // Look for one or more iframe entries containing the metaframe class.
